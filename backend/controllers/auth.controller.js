...
exports.registrarUsuario = async (req, res) => {
    ...
    try {
        // si la clave de producto de fábrica existe en la tabla dispositivos
        ...
        // si el dispositivo ya está vinculado a algún usuario
        if (dispositivo[0].usuario_id !== null) {
            return res.status(400).json({ error: "Esta clave de dispositivo ya se encuentra vinculada." });
        }

        // cifrar la clave única con bcrypt antes de guardarla en la cuenta (RNF1)
        const salt = await bcrypt.genSalt(10);
        const claveCifrada = await bcrypt.hash(claveDispositivo, salt);

        // inserción segura del nuevo usuario mediante consultas preparadas (RNF7)
        ...
        // vincular el dispositivo libre asignándole el id del nuevo usuario
        await db.execute(
